...
for i in range(options.bond_types.__len__()):
    ...
    for sh in range(shapes.__len__()):
        for j in range(shapes[sh].flags['surface_bonds'].__len__()):
            if shapes[sh].flags['surface_bonds'][j][3] == options.bond_types[i]:
                harmonic.set_coeff(options.bond_types[i], k = ktyp, r0 = shapes[sh].flags['surface_bonds'][j][2])
                _ad = True
                break
        for j in range(shapes[sh].flags['W-P_bonds'].__len__()):
            if shapes[sh].flags['W-P_bonds'][j][1] == options.bond_types[i]:
